...
# ================== 超参数配置区 ==================
# 文件路径
file_path = r'F:\妙算榜-赛题信息\CATL_容量_中值滤波\0001_LW400000003497480.csv'
...
for i, imf in enumerate(imfs):
    ...
    # 创建时间序列数据集
    X, y = create_dataset(imf_scaled, lstm_params['look_back'])
    X = X.reshape(X.shape[0], -1)  # 默认展平为 [样本数, 特征数] 形状
    ...
    # 如果是最后一个 IMF（残差序列），使用 LSTM 模型
    if i == num_imfs - 1:
        print("使用 LSTM 模型处理残差序列...")
        X_lstm = X.reshape(X.shape[0], lstm_params['look_back'], 1)  # LSTM 输入形状为 [样本数, 时间步长, 特征数]
        X_train_lstm, X_val_lstm = X_lstm[:train_size], X_lstm[train_size:]

        # 定义 LSTM 模型
        model = Sequential([
            LSTM(lstm_params['units'], input_shape=(X_train_lstm.shape[1], X_train_lstm.shape[2]), return_sequences=False),
            Dense(1)
        ])
        model.compile(optimizer='adam', loss='mse')

        # 训练 LSTM 模型
        history = model.fit(X_train_lstm, y_train, epochs=lstm_params['epochs'], batch_size=lstm_params['batch_size'], verbose=0)

        # 验证模型
        y_val_pred_scaled = model.predict(X_val_lstm).reshape(-1, 1)
        ...
        future_input = imf_scaled[-lstm_params['look_back']:].reshape(1, lstm_params['look_back'], 1)
        future_imf_pred = []

        for step in range(len(future_odometer)):
            # 预测下一个时间步的值
            next_pred_scaled = model.predict(future_input)
    ...
    # 对于其他 IMF，使用 XGBoost 模型
    else:
        print("使用 XGBoost 模型处理其他 IMF...")
        # 构建 XGBoost 模型
        model = XGBRegressor(
        ...
        # 使用最后一个窗口作为初始输入进行递归预测
        future_input = imf_scaled[-lstm_params['look_back']:].reshape(1, -1)
        future_imf_pred = []
...
final_ax.set_title('混合模型预测结果 - 叠加')
